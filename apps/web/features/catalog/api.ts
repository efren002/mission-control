...
export interface Run { id: string; objective_id: string; project_id: string; objective_title: string; repository_id: string | null; status: string; current_step: string | null; task_count: number; approval_status: string | null; created_at: string; updated_at: string }
export interface RunEvent { id: string; sequence: number; event_type: string; payload: Record<string, unknown>; created_at: string }
export interface RunInvocation { id: string; agent_id: string; agent_name: string; purpose: string; status: string; duration_ms: number | null; input_excerpt: string | null; output_excerpt: string | null; error: string | null; created_at: string; updated_at: string }
export interface RunApproval { id: string; kind: string; status: string; decision_reason: string | null; decided_at: string | null; created_at: string }
export interface RunDetail extends Run { tasks: Task[]; events: RunEvent[]; invocations: RunInvocation[]; approvals: RunApproval[] }
export interface ResumeExecution { id: string; status: string; current_step: string | null; recovered_tasks: number }
export interface Task { id: string; objective_id?: string; run_id?: string | null; title: string; description: string | null; status: string; agent_role: string; assigned_agent_id: string | null; checkpoint_sha: string | null }
export interface TaskDiff { task_id: string; commit_sha: string; diff: string; truncated: boolean }
export interface Approval { id: string; run_id: string; objective_id: string; objective_title: string; kind: string; status: string; task_count: number; decision_reason: string | null; decided_at: string | null; created_at: string; updated_at: string }
export interface Agent { id: string; name: string; role: "planner" | "developer" | "qa" | "reviewer"; provider: "codex" | "claude"; model: string | null; instructions: string; enabled: boolean; status: string; provider_version: string | null; capabilities: string[]; assignment_count: number; invocation_count: number }
...
export interface AgentInvocation { id: string; agent_id: string; run_id: string | null; task_id: string | null; purpose: string; status: string; duration_ms: number | null; input_excerpt: string | null; output_excerpt: string | null; error: string | null; created_at: string }
export interface WorkflowSettings { planner_provider: Agent["provider"]; planner_model: string | null; require_plan_approval: boolean; require_execution_approval: boolean; auto_assign_tasks: boolean; max_planning_tasks: number; provider_timeout_seconds: number; allow_repository_writes: boolean; retain_invocation_output: boolean }
...
export interface ProviderStatus { available?: boolean; installed?: boolean; authenticated?: boolean; version?: string; error?: string; authDetail?: string; capabilities?: string[]; usage?: ProviderUsage; rateLimits?: RateLimitSnapshot }
